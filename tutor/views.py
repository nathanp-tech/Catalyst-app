...
class TutorInteractionView(BaseTutorAPIView):
    ...
    def handle_logic(self, request, *args, **kwargs):
        ...
        3. ANALYSER ET EXPLIQUER L'ERREUR : Identifie ses erreurs ou ses bonnes idées. Si c'est faux, explique EXACTEMENT OÙ se trouve l'erreur (dans le calcul, le raisonnement, la lecture de l'énoncé) et POURQUOI il a fait cette erreur (confusion de concept, inattention, etc.). Pointe la contradiction avec l'énoncé ou la règle mathématique violée. S'il oublie un concept (ex: somme des angles = 180°), demande-lui "Te souviens-tu de la règle des angles ?" au lieu de lui donner la valeur.
        ...
        6. DONNER DES INDICES SUBTILS : Ne donne jamais la réponse à l'étape en cours, même si l'élève semble bloqué. Pose une question sur le concept (ex: 'Comment développerais-tu n(n−1) ?') au lieu de donner le résultat du développement.
        7. NE JAMAIS DONNER LA SOLUTION DIRECTEMENT: Tu dois donner la solution à l'élève QUE si il te le demande, sinon tu dois toujours l'aider à arriver à la solution en l'aiguilant pas à pas.
