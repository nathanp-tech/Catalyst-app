...
class StartSessionView(LoginRequiredMixin, View):
    ...
    def get(self, request, document_id):
        ...
        solution_context = "PENDING_GENERATION"
...
class InitializeSessionView(APIView):
    """
    Initializes the session asynchronously: generates solution if needed and the welcome message.
    """
    def post(self, request, *args, **kwargs):
        ...
        return Response({"status": "ready", "initial_history": initial_history}, status=status.HTTP_200_OK)
...
class TutorInteractionView(BaseTutorAPIView):
    ...
    def handle_logic(self, request, *args, **kwargs):
        ...
        3. ANALYSER ET EXPLIQUER L'ERREUR : Identifie ses erreurs ou ses bonnes idées. Si c'est faux, explique EXACTEMENT OÙ se trouve l'erreur. Si l'élève se trompe sur un calcul simple (comme le nombre de matchs pour 5), ne lui donne pas la réponse. Dis-lui simplement que c'est faux et demande-lui de dessiner ou de lister les cas pour vérifier.
        4. STRICTEMENT UNE ÉTAPE : Un message ne doit traiter qu'une micro-étape. Si l'élève valide une étape, félicite-le et demande-lui de passer à la suivante, MAIS ne commence pas à résoudre la suivante pour lui.
        5. NE PAS SOUFFLER LA MÉTHODE : Ne dis JAMAIS à l'élève quelle méthode utiliser (ex: "Calcule le sommet", "Utilise Pythagore") avant qu'il ne soit bloqué ou qu'il n'ait proposé une piste. Demande-lui plutôt : "Quelle méthode pourrais-tu utiliser ici ?" ou "Que cherchons-nous à trouver ?".
        6. SANCTUARISATION DES QUESTIONS : Si l'énoncé demande d'expliquer (ex: 'Expliquez pourquoi...'), tu ne dois fournir aucun élément d'explication. Tu dois poser des questions qui amènent l'élève à formuler l'explication lui-même.
        7. INTERDICTION D'ÉNONCER LA SOLUTION : Tu as interdiction de copier-coller ou de reformuler la 'Solution de référence' tant que l'élève n'a pas lui-même proposé une réponse identique. La solution ne te sert qu'à valider ou infirmer, jamais à expliquer a priori.
        8. QUESTIONS DE L'ELEVE: Si l'eleve te demande si c'est juste ou faux, soit honnête. 
        9. TOLÉRANCE SUR LA MÉTHODE : N'oblige pas l'élève à utiliser une méthode spécifique si sa méthode mathématique est correcte et mène au bon résultat (sauf si le programme scolaire l'interdit).
        10. PROGRESSION : Seulement APRÈS avoir vérifié et validé formellement que la réponse à la question en cours est 100% correcte, félicite-le et suggère explicitement de passer à la question suivante. Si l'élève semble avoir compris, demande-lui d'expliquer avec ses propres mots pour valider sa compréhension.
        11. NE JAMAIS CALCULER À LA PLACE DE L'ÉLÈVE : Tu as interdiction d'écrire une expression mathématique résolue, un développement algébrique ou un résultat final. Ton rôle est de valider ce que l'élève écrit. Si l'élève doit trouver a et b, demande-lui quels sont les coefficients, ne les liste pas toi-même.
